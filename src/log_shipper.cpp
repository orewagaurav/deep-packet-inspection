...
LogShipper::LogShipper(const std::string& backend_url, int batch_size, int flush_interval_ms)
    : endpoint_(backend_url + "/logs"),
      alert_endpoint_(backend_url + "/alerts"),
...
void LogShipper::enqueue(const std::string& json_line) {
    bool hit_threshold;
    {
        ...
        hit_threshold = static_cast<int>(buffer_.size()) >= batch_size_;
    }
...
// ============================================================================
// enqueueAlert() — thread-safe; alerts are low-volume, flush promptly
// ============================================================================

void LogShipper::enqueueAlert(const std::string& json_line) {
    {
        std::lock_guard<std::mutex> lock(buffer_mutex_);
        alert_buffer_.push(json_line);
    }
    // Alerts are rare and important — wake the flush thread immediately.
    buffer_cv_.notify_one();
}
...
void LogShipper::flushLoop() {
    while (running_.load()) {
...

        {
            ...
            // Wait until batch threshold, a pending alert, or timeout
            buffer_cv_.wait_for(lock, std::chrono::milliseconds(flush_interval_ms_), [this] {
                return static_cast<int>(buffer_.size()) >= batch_size_ ||
                       !alert_buffer_.empty() || !running_.load();
            });

            // Drain up to batch_size_ log entries
            while (!buffer_.empty() && static_cast<int>(batch.size()) < batch_size_) {
                batch.push_back(std::move(buffer_.front()));
                buffer_.pop();
            }
            // Drain all pending alerts (low volume)
            while (!alert_buffer_.empty()) {
                alerts.push_back(std::move(alert_buffer_.front()));
                alert_buffer_.pop();
            }
        }
        ...
        // Send alerts to the /alerts endpoint (counted with logs for stats)
        for (const auto& alert : alerts) {
            if (postJson(alert_endpoint_, alert)) {
                sent_++;
            } else {
                failed_++;
            }
...
bool LogShipper::postJson(const std::string& url, const std::string& json_body) {
...
void LogShipper::drain() {
    std::lock_guard<std::mutex> lock(buffer_mutex_);
    while (!buffer_.empty()) {
        if (postJson(endpoint_, buffer_.front())) {
            sent_++;
        } else {
            failed_++;
        }
    ...
    while (!alert_buffer_.empty()) {
        if (postJson(alert_endpoint_, alert_buffer_.front())) {
            sent_++;
        } else {
            failed_++;
        }
